...
## 2. 编译与运行 (在项目根目录下使用 PowerShell 执行)
```powershell
# 编译项目
cargo build --release
...
- **专注模式 (Focus Mode)** (已增强):
    ...
    - **操作**:
        - **多行复制 (`c`)**: 弹窗支持输入行号范围（如 `1-5`）、列表（如 `1,3,5`）或混合模式；输入 `*` / `a` / `all` 可一键复制全部。复制弹窗标题显示 "复制行号 - 请输入行号，如1-5，3，7-10，*/a/all"。
...
- **链路追踪 (Trace Filtering)**:
    ...
    - **配置支持**: 可在 `log_config.toml` 中自定义关联 ID 的正则表达试模式。
- **水平滚动与换行** (已增强):
    - **水平滚动**: 按 `h` 向左滚动，按 `l` 向右滚动，查看超长日志行内容。
    - **触摸手势**: 支持触摸板两指左右滑动进行水平滚动（在专注模式、线程视图、高级搜索浮窗中均可用）。
...
- **实时追踪 (Live Tailing)** (已增强): 支持类似 `tail -f` 的实时监控功能。当日志文件追加内容时，程序会自动检测并增量加载新行。
    - **路径处理改进**: 增强了文件监视器对 Windows 路径大小写和符号链接的处理能力。
- **噪声折叠**: 自动识别并合并连续的 USB 轮询、线程清理及完全重复的日志行，大幅提升阅读效率。
- **启动警告弹窗**: 当部分文件因权限问题无法访问时，程序会显示警告弹窗提示哪些文件跳过，并附带管理员运行提示。
- **文件筛选 (Solo Mode)** (已增强):
    - **分页导航**: `Tab` 切换到文件列表焦点后，可用 `←` / `→` 键进行翻页，标题栏显示当前页码 `[Page 1/5]`。
...
- **线程视图 (Thread View)** (已增强):
    ...
    - 保留原有功能：
        ...
        - **操作**:
            ...
            - **放大 (`+`) / 缩小 (`-`)**: 调整每行显示的高度密度。
            - **导出 (`e`)**: 导出当前线程视图的日志到文件。
            - **复制 (`c`)**: 支持行号范围复制；输入 `*` / `a` / `all` 可复制线程视图全部行。复制弹窗标题显示 "复制行号 - 请输入行号，如1-5，3，7-10，*/a/all"。
            - **退出 (`Esc` 或 `x`)**: 返回正常视图。
- **高级搜索 (Advanced Search)**:
    - **复合过滤**: `Shift+S` 打开面板，支持时间范围、内容正则、来源文件、多级级别勾选组合过滤。
...
| `/` | 搜索 | 快捷正则搜索，使用 `&` 分隔多词表示 AND 关系 (输入框显示：`Normal: keyword, Combined: keyword & keyword & ...`) (Esc退出/清除高亮，Enter应用) |
...
| `e` | 导出 | 导出当前过滤后的日志为 CSV 格式（默认保存到Windows下载目录） |
| `E` (Shift+E) | 导出 | 导出当前过滤后的日志为 JSON 格式 (支持内容自动格式化) |
...
- **权限问题**: 若提示 `拒绝访问 (os error 5)`，可能需要以管理员身份运行程序，或将日志文件复制到当前用户可访问的目录。程序会在启动警告弹窗中提示可用解决方案。
- **跳转失败**: 跳转功能使用的是“原始行号”，如果日志经过过滤，跳转会定位到最接近该行号的可视日志。

---

# LogInsight Summary (English)

## 1. Environment
- **OS**: Windows 11 (Windows Terminal recommended)
- **Toolchain**: Rust (rustc/cargo 1.75+)
- **Permissions**: Clipboard access required for `arboard`
...
## 3. Key Features
- **Smart Encoding**: Auto-detects GB18030 (Chinese) and fixes nested UTF-8 JSON strings.
- **Focus Mode**: Isolate search results with `Alt+Enter`. Supports secondary search and multi-line copy.
- **Live Tailing**: Real-time log monitoring with `f` key (Live Mode).
- **Thread View**: Press `t` to see all logs from the same thread.
- **AI Integration**: Multi-turn chat (F3) and instant diagnosis (a) via Ollama.
- **Export**: Export filtered logs to CSV (`e`) or formatted JSON (`E`). Default export path is Windows Downloads folder, configurable.
- **Performance**: Zero-copy parsing with mmap, O(visible_rows) rendering.

## 4. Quick Shortcuts
- `F1` - Logs | `F2` - Dashboard | `F3` - AI Chat | `F4` - History | `F5` - Report
- `/` - Quick Search | `Shift+S` - Advanced Search
- `w` - Toggle Word Wrap | `h/l` - Horizontal Scroll
- `f` - Toggle Live Mode | `t` - Thread View
- `p` - Attach log to AI context | `a` - AI Diagnose line
- `E` - Export to JSON (formatted)
